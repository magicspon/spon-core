 ...
 */
function loadModule({
  module, node, name, keepAlive, key,
}) {
...
function scan(fn) {
  ...
  Object.entries(list).forEach(async ([key, item]) => {
    const {
      query, name, node, hasLoaded, module, keepAlive,
    } = item;
...
export default function loadApp(context) {
  ...
  function hydrate(context) {
    sync.read(() => {
      getNodes(context)
        .filter(({ dataset: { keepAlive, spon } }) => keepAlive === 'true' || !cache.has(spon))
        .forEach((node, index) => {
          const {
            spon, query, keepAlive, ...rest
          } = node.dataset;
          if (spon.split(' ').length > 1) {
            throw new TypeError('you are only allowed to use on behaviour per dom node');
          }
  ...
  function destroy() {
    ...
    const killList = Object.values(cache.store).filter(({ keepAlive }) => !keepAlive);
